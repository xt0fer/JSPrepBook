:title-page:
= Zip Code Wilmington's Programming in JavaScript (DRAFT - NOT FOR RELEASE)
...
_As I've been thinking on it, maybe instead of a Repl.it account or a Repl on the website, its a just a simple HTML page with a code editor and Run button and a 'console' output text log showing the output from the editor's buffer? We could host such a thing on our site, and when a student was using it, it would all run inside their browser. Make it very simple, no Node or other junk to make it slow and prone to bugs?_
...
_We need a series of other resources to go on from here. Javascript.info is a really good one, IMO. I'll bet there are other decent free resources._